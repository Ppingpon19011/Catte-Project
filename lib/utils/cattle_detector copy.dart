...
class CattleDetector {
  ...
  Future<img.Image> _enhanceImageForDetection(img.Image originalImage) async {
    try {
      ...
      // 2. ทำ Noise reduction เพื่อลดสัญญาณรบกวน
      enhancedImage = img.gaussianBlur(enhancedImage, radius: 0);
      
      // 3. ปรับปรุงขอบภาพเพื่อช่วยในการตรวจจับขอบ
      enhancedImage = img.adjustColor(
        enhancedImage,
      ...
      // 4. บันทึกรูปที่ปรับปรุงแล้วเพื่อตรวจสอบ (ถ้าต้องการ)
  ...
  Future<DetectionResult> detectCattle(File imageFile) async {
    try {
      ...
      try {
        ...
        var result = await _imageToTensor(originalImage);
        ...
        try {
          ...
          try {
            ...
            if (outputShape.length == 3) {
              ...
              print('ตัวอย่างค่า [0][1][0]: ${outputBuffer[0][1][0]}');
              print('ตัวอย่างค่า [0][2][0]: ${outputBuffer[0][2][0]}');
              print('ตัวอย่างค่า [0][3][0]: ${outputBuffer[0][3][0]}');
              print('ตัวอย่างค่า [0][4][0]: ${outputBuffer[0][4][0]}');
              print('ตัวอย่างค่า [0][5][0]: ${outputBuffer[0][5][0]}');
              print('ตัวอย่างค่า [0][6][0]: ${outputBuffer[0][6][0]}');
            } else if (outputShape.length == 2) {
          ...
          // ทดลองใช้ค่า threshold ที่แตกต่างกันเพื่อหาค่าที่เหมาะสม
          List<DetectedObject> detectionResults = [];
          
          // ใช้ค่า threshold ที่ต่ำมากก่อนเพื่อรับทุกการตรวจจับที่เป็นไปได้
          double lowThreshold = 0.01;
          detectionResults = _processOutput(
            outputBuffer, 
            originalImage.width, 
            originalImage.height, 
            confidenceThreshold: lowThreshold
          );
          
          print('พบวัตถุทั้งหมด ${detectionResults.length} รายการด้วยค่า threshold $lowThreshold');
          
          // ตรวจสอบว่าพบวัตถุหรือไม่
          // if (detectionResults.isEmpty) {
          //   // ถ้าไม่พบวัตถุใดๆ เลย ให้ลองใช้ค่า threshold ที่ต่ำลงอีก
          //   double veryLowThreshold = 0.5;
          //   print('ไม่พบวัตถุ ลองใช้ค่า threshold ที่ต่ำลงเป็น ${veryLowThreshold}');
            
          //   detectionResults = _processOutput(
          //     outputBuffer, 
          //     enhancedImage.width, 
          //     enhancedImage.height, 
          //     confidenceThreshold: veryLowThreshold
          //   );
            
          //   print('พบวัตถุทั้งหมด ${detectionResults.length} รายการด้วยค่า threshold ${veryLowThreshold}');
            
          //   if (detectionResults.isEmpty) {
          //     print('ไม่พบวัตถุในภาพ แม้จะใช้ค่า threshold ต่ำมาก');
          //     return DetectionResult(
          //       success: false,
          ...
          // กรองเฉพาะวัตถุที่ต้องการ (ความยาวลำตัว, รอบอก, จุดอ้างอิง)
          List<DetectedObject> filteredObjects = detectionResults.where((obj) => 
            obj.classId >= 0 && obj.classId <= 2
          ).toList();
          ...
          // ตรวจสอบว่ามีวัตถุที่ตรวจจับได้อย่างน้อย 1 รายการ
          if (bestObjects.isEmpty) {
            return DetectionResult(
              success: false,
              error: 'ไม่พบวัตถุที่ต้องการในภาพ กรุณาวัดด้วยตนเอง',
              resizedImagePath: resizedImagePath,
            );
          }
  ...
  Future<Map<String, dynamic>> _imageToTensor(img.Image originalImage) async {
    try {
      ...
      // 4. แปลง tensor เป็นรูปแบบที่โมเดลต้องการ (NHWC format: [batch, height, width, channels])
      ...
      for (int y = 0; y < INPUT_SIZE; y++) {
        for (int x = 0; x < INPUT_SIZE; x++) {
          ...
          tensor[0][y][x][0] = pixel.r.toDouble();// / 255.0; // Red
          tensor[0][y][x][1] = pixel.g.toDouble();// / 255.0; // Green
          tensor[0][y][x][2] = pixel.b.toDouble();// / 255.0; // Blue
        }
      }
  ...
  DetectedObject _detectedObject(formattedOutput,i, imageWidth,imageHeight,classId,finalConfidence){
  ...
  List<DetectedObject> _processYoloV8Output(dynamic output, int imageWidth, int imageHeight, double confidenceThreshold) {
    ...
    try {
      ...
      // ปรับค่า threshold ให้เหมาะสมตามสถานการณ์
      print('ค่า confidence threshold: $confidenceThreshold');

      // จำนวนคลาสทั้งหมดในโมเดล
      // final int numClasses = 3; // ความยาวลำตัว (Body Length), รอบอก (Heart Girth), จุดอ้างอิง (Yellow Mark)
      ...
      for (int i = 0; i < cols; i++) {
        ...
        try {
          ...
          DetectedObject dtobj = _detectedObject(formattedOutput,i,imageWidth,imageHeight,tempIndex,maxConfidence);
          allDetectedObjects.add(dtobj);
          ...
          //   }
          // } else {
          //   // กรณีที่ไม่มีข้อมูลคลาส ใช้ค่า default
          ...
          // final double bodyLengthConf = classConfidences[0]; // ความยาวลำตัว
          // final double heartGirthConf = classConfidences[1]; // รอบอก
          // final double yellowMarkConf = classConfidences[2]; // จุดอ้างอิง

          // final List<double> confidences = [bodyLengthConf, heartGirthConf, yellowMarkConf];
          // final double maxConfidence = confidences.reduce(math.max);
          // int classId = confidences.indexOf(maxConfidence);
          
          // คำนวณความเชื่อมั่นรวม
          // double finalConfidence = hasClassData 
          //     ? maxConfidence // ใช้ค่าความเชื่อมั่นของคลาสเป็นหลัก
          //     : objectness;   // ถ้าไม่มีข้อมูลคลาส ใช้ objectness
              
          // เพิ่มการวิเคราะห์ debug
          // print('คลาส $classId: conf=$maxConfidence, obj=$objectness, final=$finalConfidence');
          ...
          //   print('ตรวจพบ $className (ClassID: $classId): ความเชื่อมั่น ${(finalConfidence * 100).toStringAsFixed(1)}%');
